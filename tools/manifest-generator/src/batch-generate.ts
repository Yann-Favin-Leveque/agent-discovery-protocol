...
interface BatchResult {
  ...
  source: "openapi" | "google-discovery" | "docs";
  error?: string;
}
...
async function main() {
  ...
  const catalog: Catalog = parseYaml(catalogRaw);
...
async function processApi(api: CatalogEntry): Promise<BatchResult> {
  ...
  try {
    ...
    if (api.openapi) {
      const source = api.type === "google-discovery" ? "google-discovery" as const : "openapi" as const;
...
function generateReport(results: BatchResult[]): BatchReport {
