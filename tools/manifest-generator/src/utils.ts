...
export async function loadSpec(input: string): Promise<unknown> {
  let raw: string;
...
export function truncateDescription(text: string, maxLen: number = 500): string {
...
export function mapParamType(schema: Record<string, unknown> | undefined, type?: string): string {
  if (!schema && !type) return "string";
  const t = (schema?.type as string) || type || "string";
  if (t === "array") {
    const itemType = (schema?.items as Record<string, unknown>)?.type as string || "string";
    return `${itemType}[]`;
  }
...
export function generateExample(name: string, type: string, schema?: Record<string, unknown>): unknown {
  ...
  switch (type) {
    case "string":
      ...
      if (nameLower.includes("url") || nameLower.includes("uri")) return "https://example.com";
      if (nameLower.includes("id")) return "abc_123";
      if (nameLower.includes("name")) return "Example";
      if (nameLower.includes("date") || nameLower.includes("time")) return "2025-01-15T10:00:00Z";
      if (nameLower.includes("phone")) return "+1234567890";
      if (nameLower.includes("currency")) return "USD";
      return "example_value";
    case "number":
      if (nameLower.includes("amount") || nameLower.includes("price")) return 100;
    ...
    case "string[]":
      if (nameLower.includes("email")) return ["user@example.com"];
      if (nameLower.includes("tag")) return ["tag1", "tag2"];
      return ["value1"];
    default:
...
export function validateManifest(manifest: Manifest): { valid: boolean; errors: string[] } {
  ...
  if (manifest.auth?.type === "oauth2") {
    if (!manifest.auth.authorization_url) errors.push("OAuth2 requires authorization_url");
    if (!manifest.auth.token_url) errors.push("OAuth2 requires token_url");
  }

  if (manifest.auth?.type === "api_key") {
    if (!manifest.auth.header) errors.push("api_key requires header");
  }
