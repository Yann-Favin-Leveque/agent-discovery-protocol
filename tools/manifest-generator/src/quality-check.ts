...
import type { Manifest, CapabilityDetail, QualityReport, QualityIssue } from "./types.js";
...
async function main() {
  ...
  for (const entry of entries) {
    ...
    console.log(
      `${icon} ${entry} — score: ${report.score}/100 (${errorCount} errors, ${warnCount} warnings)`,
    );
  }
...
async function checkQuality(domain: string, dir: string): Promise<QualityReport> {
  ...
  for (const cap of manifest.capabilities || []) {
    ...
    try {
      ...
      const detail: CapabilityDetail = JSON.parse(detailRaw);
