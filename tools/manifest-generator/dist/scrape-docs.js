...
export async function scrapeDocs(url, domain, outputDir) {
    ...
    try {
        ...
        return { success: false, capabilityCount: 0, errors: [`Fetch failed: ${err.message}`] };
    }
...
IMPORTANT: Return ONLY valid JSON, no markdown code fences.`;
    try {
        ...
        return { success: false, capabilityCount: 0, errors: [`Claude API error: ${err.message}`] };
    }
}
async function buildFromExtracted(data, domain, outputDir) {
    const errors = [];
    // Build auth
    ...
    if (data.auth) {
        ...
        if (authType === "oauth2") {
            ...
            if (data.auth.setup_url)
                auth.setup_url = data.auth.setup_url;
        }
    }
    ...
    for (const cap of (data.capabilities || []).slice(0, 12)) {
        ...
        const params = (primaryEp?.parameters || []).slice(0, 10).map((p) => ({
            name: p.name,
            type: p.type || "string",
