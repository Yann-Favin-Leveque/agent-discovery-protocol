...
import { loadSpec, toSnakeCase, truncateDescription, mapParamType, generateExample, writeJson, ensureDir, log, } from "./utils.js";
// ---- Entry point (CLI) ----
program
...
export async function convertOpenAPI(input, outputDir, domain, rewriteDescriptions = false) {
    ...
    if (spec.kind === "discovery#restDescription") {
        ...
        return convertGoogleDiscovery(rawSpec, outputDir, domain, rewriteDescriptions);
    }
    const openapi = normalizeSpec(rawSpec);
...
async function convertNormalizedSpec(spec, outputDir, domain, rewriteDescriptions) {
    const errors = [];
...
async function convertGoogleDiscovery(spec, outputDir, domain, rewriteDescriptions) {
...
function flattenGoogleResources(prefix, resources, groups, baseUrl) {
    if (!resources)
        return;
...
function googleMethodToEndpoint(method, baseUrl) {
    const params = [];
...
function determineGroup(endpoint, spec) {
    // Prefer OpenAPI tags
...
function extractEndpointInfo(path, method, op) {
    const params = [];
    ...
    for (const p of op.parameters || []) {
        ...
        if (p.in === "header" || p.in === "cookie")
            continue;
        const paramType = mapParamType(p.schema, p.type);
        params.push({
            name: p.name,
            type: paramType,
            description: p.description || `The ${p.name} parameter.`,
            required: p.required || p.in === "path",
            example: p.example ?? p.schema?.example ?? generateExample(p.name, paramType, p.schema),
        });
    }
    // Request body
            ...
            if (schema?.properties) {
                ...
                for (const [propName, propSchemaRaw] of Object.entries(schema.properties)) {
                    ...
                    params.push({
                        name: propName,
                        type: pType,
                        description: propSchema.description || `The ${propName} field.`,
...
function selectBestEndpoints(endpoints, maxCount) {
    ...
    for (const { endpoint } of scored) {
        ...
        if (result.some((r) => (r.operationId || `${r.method}:${r.path}`) === key))
            continue;
...
function buildCapabilityDescription(groupName, endpoints) {
    const actions = [];
...
function buildCapabilityDetail(name, description, primary, allEndpoints, baseUrl, auth) {
...
// ---- Base URL extraction ----
function extractBaseUrl(spec, domain) {
...
async function rewriteForLLM(text, context) {
    try {
        ...
        log.warn(`Claude API rewrite failed, using original: ${err.message}`);
    }
    return text;
}
